...
## Active Task
> One small pending item: F004 (`fixes/fix_log.md`) fixed real employee names hardcoded in
> `text-display-plugin.php`'s source LOCALLY, but that file was already live from T015's deploy
> with the old names still in it — **the corrected file needs a follow-up re-upload to
> `support.stratastaffglobal.com` (wp-content/plugins/Multi-Input-Text-Display-Plugi/) whenever the
> user is next in cPanel.** Not urgent (site works fine either way, this is a labels-only PII
> cleanup) but shouldn't be forgotten.
...
| F004 | Post-deploy PII audit: grepped all tracked repo files (not just this session's) for the 5 employee names and found them hardcoded in `Multi-Input-Text-Display-Plugi/text-display-plugin.php`'s own `tdp_get_person_name()` function — present since this repo's first commit, predating this session and the earlier T014 doc-only redaction. Replaced with generic labels ("Agent 1"–"Agent 5"), a display-label-only change (no option keys or saved data touched). Fixed locally and pushed to GitHub; **the live site still has the old file from the T015 deploy** — needs a follow-up re-upload, tracked in Active Task above | 2026-07-15 |
